...
public sealed class ProductImageService(IOptions<StorageOptions> options) : IProductImageService
{
    ...
    private static string ResolveRoot(string configured)
    {
        ...
        return Path.GetFullPath(Path.Combine(root, "products"));
    }

    public async Task<string> SaveAsync(int productId, Stream content, string contentType, CancellationToken cancellationToken = default)
    {
        ...
        var relativePath = $"{productId}-{Guid.NewGuid():N}{extension}";
