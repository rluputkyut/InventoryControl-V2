...
public sealed class ShopImageService(IOptions<StorageOptions> options) : IShopImageService
{
    ...
    private static string ResolveRoot(string configured)
    {
        ...
        return Path.GetFullPath(Path.Combine(root, "shops"));
    }

    public async Task<string> SaveAsync(int shopId, Stream content, string contentType, CancellationToken cancellationToken = default)
    {
        ...
        var relativePath = $"{shopId}-{Guid.NewGuid():N}{extension}";
