...
/// <summary>Stores delivery/payment logo images under `uploads/refs`.</summary>
public sealed class ReferenceImageService(IOptions<StorageOptions> options) : IReferenceImageService
{
    ...
    private static string ResolveRoot(string configured)
    {
        ...
        return Path.GetFullPath(Path.Combine(root, "refs"));
    }

    public async Task<string> SaveAsync(int referenceId, Stream content, string contentType, CancellationToken cancellationToken = default)
    {
        ...
        var relativePath = $"{referenceId}-{Guid.NewGuid():N}{extension}";
    ...
    public Task<Stream?> OpenAsync(string imagePath, CancellationToken cancellationToken = default)
    {
        Stream? stream = null;
        try
        {
            var fullPath = SafePath(imagePath);
